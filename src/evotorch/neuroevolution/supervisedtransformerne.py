...
from ..core import BoundsPairLike
from ..tools.misc import Device
from . import SupervisedNE


class SupervisedTransformerNE(SupervisedNE):
    """
    Representation of a neuro-evolution problem for a transformer based model
    where the goal is to minimize a loss function in a supervised learning setting.

    Must pip install the following HuggingFace packages: peft, transformers, and datasets
    """
    ...
    def __init__(
        ...
        *,
        ...
        """
        `__init__(...)`: Initialize the SupervisedTransformerNE.
    ...
    def _evaluate_using_minibatch(self, network: nn.Module, batch: Any) -> Union[float, torch.Tensor]:
        ...
        with torch.no_grad():
            x, mask, y = batch
            yhat = network(x, mask)
            return self.loss(yhat.logits, y)
